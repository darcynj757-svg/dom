...
import ScrollHouseBlock from "@/components/ScrollHouseBlock";
import { useHeroParallax } from "@/hooks/useHeroParallax";
import { useIsMobile } from "@/hooks/useIsMobile";
import { PROJECTS } from "@/data/projects";
import { GALLERY_ITEMS } from "@/pages/gallery";
import { ARTICLES_META as ARTICLES } from "@/data/articles-meta";
import heroVideo from "@assets/cedar_log_mansion_flythrough_16s_hq.mp4";
import heroPoster from "@assets/hero-poster.webp";
const heroPhoto = "/images/gallery/photo-1.webp";
...
function FeaturedProjectCard({
  ...
  return (
    <motion.div
      className="snap-center shrink-0 w-[78vw] md:w-[420px] relative"
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, root: scrollRef, margin: "0px" }}
      transition={{ duration: 0.5, delay: index * 0.07, ease: [0.25, 0.46, 0.45, 0.94] }}
    >
      <Link href={`/projects/${project.id}`}>
        <div className="group relative overflow-hidden rounded-2xl cursor-pointer shadow-md hover:shadow-xl active:shadow-xl transition-shadow duration-300">
          {/* Image */}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5">
            ...
            <h3 className="mt-1 font-display text-base md:text-lg font-bold text-white leading-snug">{project.title}</h3>
...
export default function Home() {
  useSeo({
    title: "Деревянные дома и бани под ключ в Томске",
    description: "Кедр Томск — строительство деревянных домов из профилированного бруса и рубленого бревна с 2001 года. Более 200 объектов по всей России. Бесплатный проект при заказе.",
  });

  // Hero text appears only after the splash logo animation finishes
  const [heroReady, setHeroReady] = useState(false);
  useEffect(() => {
    // If splash was already removed (e.g. navigating back to home), show immediately
  ...
  const { sectionRef: heroRef, y: heroY } = useHeroParallax();
  const heroVideoRef = useRef<HTMLVideoElement>(null);

  // Сигнализируем splash-экрану что контент готов:
  // — на десктопе: когда hero-видео может воспроизводиться (onCanPlay на video)
  // — на мобиле: сигналим по таймауту, т.к. браузеры часто задерживают canplay
  useEffect(() => {
    if (isMobile) {
      // Даём короткое время на попытку загрузки, затем скрываем splash
      const t = setTimeout(() => window.dispatchEvent(new CustomEvent("hero-video-ready")), 300);
      return () => clearTimeout(t);
    }
  }, [isMobile]);

  // Программный play() — fallback для iOS Safari и Android Chrome,
  // которые могут игнорировать атрибут autoplay без явного вызова
  useEffect(() => {
    ...
    // Пробуем сразу
    tryPlay();
    // И после первого взаимодействия пользователя (на случай политики браузера)
  ...
  // Non-passive wheel listener on the projects carousel so vertical scrolls
  // pass through to the page instead of being swallowed by overflow-x-auto.
  useEffect(() => {
  ...
  // Lazy-load the CTA video — only start buffering when it nears the viewport.
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <section ref={heroRef as React.RefObject<HTMLElement>} className="relative h-screen min-h-[600px] overflow-hidden bg-[#1c1a17]">
        <motion.div className="absolute inset-x-0 -top-[20%] h-[140%] will-change-transform" style={{ y: heroY }}>
          <video
            ref={(el) => {
          ...
          >
            {/* Сжатая версия для мобильных (6 МБ вместо 15 МБ) */}
            {isMobile && <source src="/videos/hero-compressed.mp4" type="video/mp4" />}
      ...
      <section className="py-20 md:py-28 bg-muted/40">
        <div className="container mx-auto px-4 md:px-6">
          ...
          {/* Horizontal swipe carousel on all screen sizes */}
          <div
            ref={carouselRef}
      ...
      <section className="py-20 md:py-28 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          >
            ...
            <h2 className="mt-3 font-display text-2xl md:text-4xl lg:text-5xl font-black">Почему выбирают нас</h2>
          </motion.div>
      ...
      <section className="py-20 md:py-28 bg-muted/30">
        <div className="container mx-auto px-4 md:px-6">
          ...
          >
            ...
            <h2 className="mt-3 font-display text-2xl md:text-4xl lg:text-5xl font-black">Как начать строительство</h2>
        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          >
            <div>
              ...
              <h2 className="mt-3 font-display text-2xl md:text-4xl lg:text-5xl font-black">Реализованные проекты</h2>
